...
'''
Note:You can't initialize `dp` as `dp = triangle[-1]` directly because it creates a reference to the same list object as the last row of triangle.
This means any modifications to `dp` will also modify `triangle[-1]`, which is not desirable since you want dp to be an independent array that you can modify freely.

To ensure dp is a separate list that can be modified without affecting triangle, you should create a copy of the last row of triangle. 
This can be done using list slicing, as shown here `dp = triangle[-1][:]`


`dp = deepcopy(triangle[-1])`
Using deepcopy from the copy module is another valid way to ensure that dp is a separate object from triangle[-1]. 
However, since the elements of triangle[-1] are integers (which are immutable in Python), a shallow copy created by dp = triangle[-1][:] or dp = triangle[-1][::] is sufficient. 

Using deepcopy is typically more useful when dealing with nested lists or mutable objects within the list.
'''
