...
class Solution:
    def findMin(self, nums: List[int]) -> int:
         ...
         while l<r:                # Continue until the search range is exhausted
            ...
            if nums[mid]>nums[r]: #Note we are not searching for target (i.e nums[r]) this time we are searching for the smallest value in the rotated sorted array that why we are going to update the right pointer to mid
                l = mid+1 
            else:
                r = mid    # Otherwise, the minimum is on the left or at mid
        return nums[l]   