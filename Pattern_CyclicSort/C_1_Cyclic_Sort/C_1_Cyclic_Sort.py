...
class Solution:
    def sort(self,nums):
        i = 0
        while i < len(nums):
            pos = nums[i] - 1 #Calculate the index where the current element should be place
            if nums[i]!=nums[pos]: # Check if the current element is not in its correct position.
                nums[i],nums[pos]=nums[pos],nums[i] # Swap the current element with the one at its correct position.
            else:
